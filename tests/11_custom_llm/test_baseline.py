...
def test_custom_llm_baseline_matches_declared_configuration():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: custom llm baseline matches declared configuration.
    ...
    -----------------------
    Le comportement lié à custom model baseline doit être observable avec un exemple minimal et déterministe.
    ...
    ----------------------
    Commencer avec torch.manual_seed(0), de petits tenseurs CPU et torch.testing.assert_close si flottant. Code cible: src/inference_lab/models/custom_llm/.
