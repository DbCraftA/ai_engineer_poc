...
def test_tensor_can_be_created_on_cuda():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: tensor can be created on cuda.
    ...
    -----------------------
    Le comportement lié à device CUDA doit être observable avec un exemple minimal et déterministe.
    ...
    ----------------------
    Commencer avec torch.manual_seed(0), de petits tenseurs CPU et torch.testing.assert_close si flottant. Code cible: helpers GPU.
...
def test_cpu_and_cuda_tensors_report_different_devices():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: cpu and cuda tensors report different devices.
