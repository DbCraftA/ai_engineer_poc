...
def test_nanochat_like_config_defines_complete_architecture():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: nanochat like config defines complete architecture.
    ...
    -----------------------
    Le comportement lié à nanochat-like config doit être observable avec un exemple minimal et déterministe.
    ...
    ----------------------
    Commencer avec torch.manual_seed(0), de petits tenseurs CPU et torch.testing.assert_close si flottant. Code cible: src/inference_lab/models/nanochat_like/config.py.
