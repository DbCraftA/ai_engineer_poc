...
def test_nanochat_like_model_can_overfit_tiny_batch():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: nanochat like model can overfit tiny batch.
    ...
    -----------------------
    Le comportement lié à training doit être observable avec un exemple minimal et déterministe.
    ...
    ----------------------
    Commencer avec torch.manual_seed(0), de petits tenseurs CPU et torch.testing.assert_close si flottant. Code cible: modèle/training.
