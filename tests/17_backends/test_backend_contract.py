...
def test_backend_exposes_required_tensor_and_execution_capabilities():
    ...
    Objectif
    --------
    Spécifier clairement la connaissance: backend exposes required tensor and execution capabilities.
    ...
    -----------------------
    Le comportement lié à backend contract doit être observable avec un exemple minimal et déterministe.
    ...
    ----------------------
    Commencer avec torch.manual_seed(0), de petits tenseurs CPU et torch.testing.assert_close si flottant. Code cible: future backends.
